...
def load(p):
    ...
    ch=struct.unpack('<H',b[22:24])[0]; bits=struct.unpack('<H',b[34:36])[0]
    if fmt==3: v=np.frombuffer(d,'<f4').astype(float)
    elif bits==24:
        a=np.frombuffer(d[:len(d)//3*3],np.uint8).reshape(-1,3).astype(np.int32)
        w=(a[:,0]|(a[:,1]<<8)|(a[:,2]<<16)); v=np.where(w&0x800000,w-(1<<24),w)/8388608.0
    else: v=np.frombuffer(d,'<i2').astype(float)/32768
    if ch>1: v=v[::ch]            # de-interleave: take channel 0 (L)
    return v
